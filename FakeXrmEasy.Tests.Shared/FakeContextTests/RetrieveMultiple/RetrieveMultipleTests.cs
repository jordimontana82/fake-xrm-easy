...
namespace FakeXrmEasy.Tests.FakeContextTests.RetrieveMultiple
{
    public class RetrieveMultipleTests
    {
        ...
        [Fact]
        public void Link_Entity_Works_With_AutoGenerated_Metadata()
        {
            var context = new XrmFakedContext();
            context.AutoGenerateMetadata = true;
            var contact = new Entity("contact")
            {
                Id = Guid.NewGuid(),
                ["fullname"] = "Willy Coyote"
            };
            ...
            var query = new QueryExpression("account");
            query.AddLink("contact", "primarycontactid", "contactid");

            var result = context.GetOrganizationService().RetrieveMultiple(query);

            Assert.True(result.Entities.Any());
        }

        [Fact]
        public void Link_Entity_Get_Throws_If_Attribute_Does_Not_Exist()
        {
            var context = new XrmFakedContext();
            context.AutoGenerateMetadata = true;
            var contact = new Entity("contact")
            ...
            var query = new QueryExpression("account");
            query.AddLink("contact", "contactid", "primarycontactid");

            Assert.Throws<FaultException<OrganizationServiceFault>>(() => context.GetOrganizationService().RetrieveMultiple(query));
        }
