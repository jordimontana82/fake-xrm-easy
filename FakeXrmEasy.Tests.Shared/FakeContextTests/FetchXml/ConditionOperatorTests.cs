...
namespace FakeXrmEasy.Tests.FakeContextTests.FetchXml
{
    ...
    public class ConditionOperatorTests
    {
        ...
        [Fact]
        public void FetchXml_Operator_InFiscalYear_NotCurrentFiscalYear()
        {
            var today = DateTime.Today;
            var thisYear = today.Year;
            ...
            var fetchXml = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                              <entity name='contact'>
                                    <attribute name='anniversary' />
                                        <filter type='and'>
                                            <condition attribute='anniversary' operator='in-fiscal-year' value='{thisYear + 1}' />
                                        </filter>
                                  </entity>
                            </fetch>";

            var ct1 = new Contact() { Id = Guid.NewGuid(), Anniversary = new DateTime(thisYear, 1, 2) }; // Second day of this year - should not be returned
            var ct2 = new Contact() { Id = Guid.NewGuid(), Anniversary = new DateTime(thisYear, 12, 31) }; // Last day of this year - should not be returned
            var ct3 = new Contact() { Id = Guid.NewGuid(), Anniversary = new DateTime(thisYear + 1, 1, 2) }; // Second day of next year - should be returned
            ctx.Initialize(new[] { ct1, ct2, ct3 });
            var service = ctx.GetOrganizationService();
