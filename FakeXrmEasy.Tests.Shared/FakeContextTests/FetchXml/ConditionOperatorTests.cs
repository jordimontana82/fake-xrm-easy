        ...
        public void FetchXml_Operator_Last_X_Weeks_Execution()
        {
            ...
            var fetchXml = @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                              <entity name='contact'>
                                    <attribute name='anniversary' />
                                        <filter type='and'>
                                            <condition attribute='anniversary' operator='last-x-weeks' value='3' />
                                        </filter>
                                  </entity>
                            </fetch>";

            var date = DateTime.Now;
            var ct1 = new Contact() { Id = Guid.NewGuid(), Anniversary = date.AddDays(- 14) }; //Should be returned
            var ct2 = new Contact() { Id = Guid.NewGuid(), Anniversary = date.AddDays(- 28) }; //Should not be returned
            ctx.Initialize(new[] { ct1, ct2 });
            var service = ctx.GetOrganizationService();
