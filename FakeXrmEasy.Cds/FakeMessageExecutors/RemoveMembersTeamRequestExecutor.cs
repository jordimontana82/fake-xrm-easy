...
namespace FakeXrmEasy.FakeMessageExecutors
{
    public class RemoveMembersTeamRequestExecutor : IFakeMessageExecutor
    {
        ...
        public OrganizationResponse Execute(OrganizationRequest request, XrmFakedContext ctx)
        {
            ...
            if (team == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason(string.Format("Team with Id {0} wasn't found", req.TeamId.ToString())));
            }
            ...
            foreach (var memberId in req.MemberIds)
            {
                ...
                if (user == null)
                {
                    throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason(string.Format("SystemUser with Id {0} wasn't found", memberId.ToString())));
                }
